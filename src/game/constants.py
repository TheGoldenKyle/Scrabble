# WINDOW SETTINGS #
SCREEN_WIDTH = 740
SCREEN_HEIGHT = 910
BACKGROUND_IMAGE = "resources/background.png"

# GAME VALUES #
STARTING_RANDOMS = 8
MIN_STARTING_WORDS = 10
MIN_WORD_SIZE = 2
TILE_SIZE = 65
BOARD_ROWS = int((SCREEN_HEIGHT - 120) / (TILE_SIZE + 1.3636))
BOARD_COLS = int((SCREEN_WIDTH - 10) / (TILE_SIZE + 1.3636))


DRAGGED_TILE_SIZE = 83

LETTER_VALUES = {'E': 1, 'A': 1, 'I': 1, 'O': 1, 'N': 1, 'R': 1, 'T': 1, 'L': 1, 'S': 1, 'U': 1, 'D': 2, 'G': 2,
                 'B': 3, 'C': 3, 'M': 3, 'P': 3, 'F': 4, 'H': 4, 'V': 4, 'W': 4, 'Y': 4, 'K': 5, 'J': 8, 'X': 8,
                 'Q': 10, 'Z': 10, ' ': 0}

LETTER_LOCATIONS = {'A': "2,2", 'B': "83,2", 'C': "164,2", 'D': "245,2", 'E': "326,2", 'F': "407,2", 'G': "488,2",
                    'H': "2,83", 'I': "83,83", 'J': "164,83", 'K': "245,83", 'L': "326,83", 'M': "407,83", 'N': "488,83",
                    'O': "2,164", 'P': "83,164", 'Q': "164,164", 'R': "245,164", 'S': "326,164", 'T': "407,164", 'U': "488,164",
                    'V': "2,245", 'W': "83,245", 'X': "164,245", 'Y': "245,245", 'Z': "326,245", ' ': "407,245"}

# UI VALUES #
TILE_TEXTURES = "resources/letters.png"

FONT = "times_new_roman"
FONT_SIZE = 24

ARROW_IMAGE = "resources/arrow.png"
ARROW_CLICKED_IMAGE = "resources/arrow_clicked.png"
ARROW_SIZE = 79
ARROW_X, ARROW_Y = 8 * 81 + 6, 9 * 83 + 65

BACK_ARROW_IMAGE = "resources/backarrow.png"
BACK_ARROW_CLICKED_IMAGE = "resources/backarrow_clicked.png"
BACK_ARROW_SIZE = 55
BACK_ARROW_X, BACK_ARROW_Y = 8 * 81 + 22, 3
